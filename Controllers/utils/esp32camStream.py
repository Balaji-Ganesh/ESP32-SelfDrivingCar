# utility functions of ESP32 camera stream handling
import requests
...
def set_brightness(url: str, value: int = 1, verbose: bool = False):
    try:
...
def set_led_intensity(url: str, value: int = 1, verbose: bool = False):
    try:
        if value >= -2 and value <= 2:
            requests.get(url + "/control?var=led_intensity&val={}".format(value))
    except:
        print("SET_LED_INTENSITY: something went wrong")
